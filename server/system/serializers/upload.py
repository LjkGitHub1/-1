...
class UploadFileSerializer(BaseModelSerializer):
    class Meta:
        model = UploadFile
        fields = ['pk', 'filename', 'filesize', 'mime_type', 'file_type', 'md5sum', 'file_url', 'access_url', 'is_tmp', 'is_upload', 'description', 'user', 'created_time']
        read_only_fields = ["pk", "is_upload", "created_time"]
        table_fields = ['pk', 'filename', 'filesize', 'file_type', 'access_url', 'is_tmp', 'is_upload', 'md5sum', 'description', 'user', 'created_time']
        extra_kwargs = {
            'user': {'required': False, 'attrs': ['pk', 'username', 'nickname', 'medical_record_no'], 'format': "{nickname}({username})"},
        }
