...
def recursion_urls(pre_namespace, pre_url, urlpatterns, url_ordered_dict):
    ...
    """
    logger.info(f"Processing URLs for namespace: {pre_namespace}, pre_url: {pre_url}")
    for item in urlpatterns:
        if isinstance(item, URLPattern):
            # print(f"Found URL pattern: {item.name}, url: {pre_url + str(item.pattern)}")
            if not item.name:
                continue
            ...
            print(f"Found URL resolver: namespace={item.namespace}, pattern={item.pattern}")
            new_pre_url = pre_url + item.pattern.regex.pattern.lstrip('^')
            print(check_show_url(new_pre_url), f"pre_url: {new_pre_url}")
