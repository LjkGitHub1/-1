...
def get_user_menu_queryset(user_obj):
    ...
    if user_obj.roles.exists():
        ...
        has_role = True
    dept = getattr(user_obj, 'dept', None)
    if dept:
        q |= (Q(userrole__deptinfo=dept) & Q(userrole__deptinfo__is_active=True))
        has_role = True
    if has_role:
...
def get_user_field_queryset(user_obj, menu):
    ...
    dept = getattr(user_obj, 'dept', None)
    if dept:
        q |= (Q(role__deptinfo=dept) & Q(role__deptinfo__is_active=True))
        has_q = True
    if has_q:
