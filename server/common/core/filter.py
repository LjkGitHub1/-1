...
def get_filter_queryset(queryset: QuerySet, user_obj: UserInfo):
    ...
    # 如果是匿名用户（未认证用户），直接返回所有数据（用于白名单接口）
    if not user_obj or not hasattr(user_obj, 'is_authenticated') or not user_obj.is_authenticated:
        logger.info(f"anonymous user. return all queryset {queryset.model._meta.label_lower}")
        return queryset
    ...
    dept_obj = getattr(user_obj, 'dept', None)
    q = Q()
...
class BaseDataPermissionFilter(BaseFilterBackend):
    def filter_queryset(self, request, queryset, view):
        # 安全地获取用户对象，如果是匿名用户则传入 None
        user_obj = request.user if (request.user and hasattr(request.user, 'is_authenticated') and request.user.is_authenticated) else None
        return get_filter_queryset(queryset, user_obj)
